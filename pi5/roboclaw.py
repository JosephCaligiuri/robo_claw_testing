...
class Roboclaw:
	...
	def read_pwm_values(self, address):
		self._sendcommand(address, 48)

		response = self._port.read(6) 
		if len(response) == 6:
			m1_pwm = int.from_bytes(response[0:2], byteorder='little', signed=True)
			m2_pwm = int.from_bytes(response[2:4], byteorder='little', signed=True)

			m1_duty_cycle_percent = m1_pwm / 327.67
			m2_duty_cycle_percent = m2_pwm / 327.67

			return m1_duty_cycle_percent, m2_duty_cycle_percent
		else:
			print("Invalid response length")
			return None
		
	def drive_motor_m1(self, address, duty_cycle):
		duty_bytes = duty_cycle.to_bytes(2, byteorder="little", signed=True)

		command = bytes([32]) + duty_bytes
		self._sendcommand(address, command)
	...
	def _writechecksum(self):
		...
		if(len(val)>0):
			if val[0]:
				print("true" + str(len(val))) #debug
				return True
		print("false" + str(len(val))) #debug
		return False
